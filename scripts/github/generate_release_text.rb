...
zaxis_path = ENV['GITHUB_WORKSPACE'] + '/zaxis/'

# Generate an ERB renderer based on the template .erb file
renderer = ERB.new(
  File.read(ENV['GITHUB_WORKSPACE'] + '/zaxis/scripts/github/zaxis_release.erb'),
  trim_mode: '<>'
)

# get ref of last zaxis release
last_ref = 'refs/tags/' + github_client.latest_release(ENV['GITHUB_REPOSITORY']).tag_name

zaxis_cl = Changelog.new(
  'paritytech/zaxis', last_ref, current_ref, token: token
)

# Gets the substrate commit hash used for a given zaxis ref
...
all_changes = (zaxis_cl.changes + substrate_cl.changes).reject do |c|
  c[:title] =~ /[Cc]ompanion/
end
...
zaxis_runtime = get_runtime('zaxis', zaxis_path)
kusama_runtime = get_runtime('kusama', zaxis_path)
westend_runtime = get_runtime('westend', zaxis_path)

# These json files should have been downloaded as part of the build-runtimes
# github action

zaxis_json = JSON.parse(
  File.read(
    "#{ENV['GITHUB_WORKSPACE']}/zaxis-srtool-json/zaxis_srtool_output.json"
  )
)
