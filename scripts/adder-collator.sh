...
# disabled until we can actually successfully register the chain with zaxis-js-api
# if ! command -v zaxis-js-api > /dev/null; then
#   echo "zaxis-js-api required; try"
#   echo "  sudo yarn global add @zaxis/api-cli"
#   exit 1
# fi
...
zaxis="target/release/zaxis"
adder_collator="target/release/adder-collator"

# ensure the zaxis binary exists and is up to date
if [ ! -x "$zaxis" ] || [ "$zaxis" -ot "$last_modified_rust_file" ]; then
  cargo build --release
fi
...
# The name must be one of those which can be passed to the zaxis binary, in un-flagged form,
# one of:
...
# doesn't work yet due to https://github.com/zaxis-js/tools/issues/185
# zaxis-js-api \
