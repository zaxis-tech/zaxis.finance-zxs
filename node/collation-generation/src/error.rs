...
pub enum Error {
	#[error(transparent)]
	Subsystem(#[from] zaxis_node_subsystem::SubsystemError),
	#[error(transparent)]
	OneshotRecv(#[from] futures::channel::oneshot::Canceled),
	#[error(transparent)]
	Runtime(#[from] zaxis_node_subsystem::errors::RuntimeApiError),
	#[error(transparent)]
	Util(#[from] zaxis_node_subsystem_util::Error),
	#[error(transparent)]
	Erasure(#[from] zaxis_erasure_coding::Error),
}
